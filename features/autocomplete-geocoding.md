...
| T02 | Gõ thêm ký tự / xóa ký tự (text thay đổi)                | Web, Mobile | Text string mới                           | B01 |
...
| `zero_results` | Không có gợi ý phù hợp                  | "Không tìm thấy gợi ý nào"       |
...
And    mỗi suggestion gồm: icon (loại địa điểm), primary text (tên/địa chỉ), secondary text (thành phố/tỉnh)
And    location bias áp dụng: ưu tiên kết quả gần map center

Given  người dùng gõ thêm hoặc xóa ký tự
...
And    Search Bar input cập nhật text của item được highlight

When   nhấn [↑]
Then   highlight lên item trước
And    nếu đã ở item đầu tiên → về lại text người dùng đã gõ

...
| #  | USP                                    | Mô tả                                                                                     | So sánh                                                |
| -- | -------------------------------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| 1  | Multi-modal bao gồm Flight             | Hỗ trợ cả chuyến bay liên tỉnh/quốc tế trong cùng một giao diện chỉ đường                | Google Maps không có mode Flight trong Directions      |
| 2  | Explore new places along this route    | Khám phá POI dọc tuyến đường đã chọn — tăng engagement và discoverability                 | Google Maps có "Search along route" tương tự           |
| 3  | Explore nearby điểm đến               | Đến nơi xong, khám phá ngay POI xung quanh destination — trải nghiệm liền mạch           | Google Maps cần quay lại search để tìm nearby          |
| 4  | Traffic realtime dữ liệu nội địa      | Dữ liệu traffic từ nguồn VN (camera giao thông, cảm biến) — chính xác hơn cho VN         | Google Maps dùng data từ Android users, hạn chế ở VN   |
| 5  | Transit data xe buýt / metro VN        | Tích hợp dữ liệu phương tiện công cộng VN (BRT, metro HCM/HN) cập nhật từ cơ quan vận tải | Google Maps transit data VN chưa đầy đủ                |
| 6  | Self-hosted routing, không giới hạn    | Routing engine tự host — không bị rate limit, phù hợp Fleet/Logistics gọi hàng nghìn route/ngày | Google Directions API tính phí per-request             |

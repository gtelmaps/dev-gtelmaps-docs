...
| #   | Loại tìm kiếm               | Ví dụ                                                                                  | Service           |
| --- | --------------------------- | -------------------------------------------------------------------------------------- | ----------------- |
| 1   | Địa chỉ đầy đủ (mới + cũ)   | `136 Nguyễn Huệ, Phường Bến Nghé, TP.HCM` (bỏ cấp quận huyện theo địa chỉ mới)         | Forward Geocoding |
| 2   | Tên đường                   | `Nguyễn Huệ`                                                                           | Forward Geocoding |
| 3   | Tìm lân cận                 | `quán ăn gần đây`, `ATM gần tôi`                                                       | Search Nearby     |
| 4   | Địa điểm / POI              | `Bitexco`, `Bưu điện thành phố`                                                        | Forward Geocoding |
| 5   | Tọa độ WGS84 (nhiều format) | DD: `10.7769, 106.7009` · DMS: `10°46'36.8"N 106°42'3.2"E` · UTM: `48P 694251 1191866` | Reverse Geocoding |
| 6   | Tọa độ VN2000               | `VN2000: 1191866, 694251`                                                              | Reverse Geocoding |
| 7   | Plus Code                   | `7P28QPG4+4R`                                                                          | Reverse Geocoding |
| 8   | Postal Code                 | `700000`                                                                               | Forward Geocoding |
| 9   | Giao lộ                     | `Nguyễn Huệ × Lê Lợi`                                                                  | Forward Geocoding |
| 10  | Công trình xây dựng         | Entrance / cổng, cầu, hầm: `Cầu Thủ Thiêm`, `Hầm Thủ Thiêm`                            | Forward Geocoding |
| 11  | Ranh giới hành chính        | Quốc gia, tỉnh/thành, quận/huyện, phường/xã: `Quận 1`, `Tỉnh Bình Dương`               | Forward Geocoding |
| 12  | Thương hiệu                 | `Highlands Coffee`, `Circle K`                                                         | Forward Geocoding |
| 13  | Chỉ đường                   | `từ Bitexco đến sân bay Tân Sơn Nhất`                                                  | Directions        |
...
| #   | USP                                  | Mô tả                                                                                      | So sánh                                                            |
| --- | ------------------------------------ | ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------ |
| 1   | 13 loại query trong 1 ô input        | Địa chỉ, tọa độ (WGS84/VN2000), Plus Code, Postal Code, giao lộ, thương hiệu, chỉ đường... | Google Maps hỗ trợ ít loại hơn (không có VN2000, Plus Code native) |
| 2   | Hỗ trợ địa chỉ mới VN                | Nhận diện cả địa chỉ mới (bỏ cấp quận huyện) và địa chỉ cũ                                 | Google Maps chưa cập nhật format địa chỉ mới VN                    |
| 3   | VN2000 + Plus Code native            | Nhập trực tiếp tọa độ VN2000 hoặc Plus Code — phục vụ ngành GIS, xây dựng, bưu chính       | Google Maps chỉ hỗ trợ WGS84 DD và Plus Code                       |
| 4   | Detect intent chỉ đường từ text      | Gõ "từ A đến B" → tự chuyển sang Directions mode, không cần mở panel riêng                 | Google Maps không detect intent chỉ đường từ search                |
| 5   | Tìm giao lộ                          | Nhập "Nguyễn Huệ × Lê Lợi" → trả về điểm giao cắt chính xác                                | Google Maps không hỗ trợ tìm giao lộ trực tiếp                     |
| 6   | Tìm ranh giới hành chính + fitBounds | Nhập "Quận 1" → hiển thị ranh giới polygon + fitBounds khu vực                             | Google Maps chỉ trả về điểm center, không vẽ boundary              |
| 7   | Vietnamese diacritics tolerant       | Gõ không dấu "nguyen hue" vẫn tìm đúng "Nguyễn Huệ"                                        | Google Maps cũng hỗ trợ nhưng kém chính xác cho VN                 |
